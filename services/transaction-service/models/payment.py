...
class Payment(db.Model):
    ...
    payment_method = db.Column(db.Enum('e-wallet', 'cash', 'bank'), nullable=False)
    amount = db.Column(db.Float, nullable=False) # = final_price - fee
    payment_status = db.Column(db.Enum('initiated', 'completed', 'failed'), default='initiated', nullable=False)
    # completed = paid, failed = deleted transaction, initiated = pending
